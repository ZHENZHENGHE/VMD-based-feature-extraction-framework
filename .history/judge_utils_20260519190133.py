...
def plot_physio_event_comparison(
    df_out,
    event_metrics,
    event_id,
    margin_s=10.0,
    ...
    row = event_metrics[event_metrics["EventID"] == event_id].iloc[0]

    start = row["StartTime"] - margin_s
    end = row["EndTime"] + margin_s

    seg = df_out[
        (df_out[time_col] >= start) &
        (df_out[time_col] <= end)
    ]
    ...
    event_region = seg["PhysioEventMask"]

    plt.scatter(
        seg.loc[event_region, "Time"],
        seg.loc[event_region, "Pressure_Clean"],
        s=20,
        label="PhysioEventMask"
    )

    plt.title(
        f"Event {event_id}: "
        f"Peak ratio={row['PeakPreservationRatio']:.3f}, "
        f"Area ratio={row['AreaPreservationRatio']:.3f}, "
        f"Corr={row['ShapeCorrelation']:.3f}"
    )
